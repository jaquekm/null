import { Bell, BookOpen, CalendarClock, CircleDollarSign, DatabaseBackup, Download, FileScan, KeyRound, Keyboard, Link2, ListChecks, Package, PiggyBank, Plug, Shapes, Shield, Smartphone, Sparkles, Tag, Trash2, Upload, Wallet, Zap } from "lucide-react";
import Link from "next/link";
import { countFailedJobs } from "@/features/jobs/queries";
import { ensureCleanupSchedule, ensureOpsDailyCheckSchedule } from "@/features/ops/queries";
import { requireOwner } from "@/lib/auth";

export default async function ConfiguracoesPage() {
  const { supabase, user } = await requireOwner();
  await ensureOpsDailyCheckSchedule(supabase, user.id);
  await ensureCleanupSchedule(supabase, user.id);
  ...
  return (
    <div className="mx-auto flex max-w-xl flex-col gap-6 p-6">
      ...
      <Link
        href="/configuracoes/manual"
        className="flex items-center gap-3 rounded-lg border border-black/[.08] px-4 py-3 text-sm text-zinc-700 transition-colors hover:bg-black/[.04] dark:border-white/[.08] dark:text-zinc-200 dark:hover:bg-white/[.06]"
      >
        <BookOpen className="h-5 w-5 shrink-0" />
        <div>
          <p className="font-medium text-black dark:text-zinc-50">Manual do sistema</p>
          <p className="text-zinc-500 dark:text-zinc-400">Como usar, runbook de incidentes e onde estão os segredos</p>
        </div>
      </Link>
