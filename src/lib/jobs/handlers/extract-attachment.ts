...
async function ocrPdfBytes(bytes: Uint8Array, ownerId: string, itemId: string): Promise<string> {
...
async function ocrPdf(bytes: Uint8Array, totalPages: number, ownerId: string, itemId: string): Promise<string> {
...
export const extractAttachment: JobHandler = async (job, { supabase }) => {
  ...
  if (!attachment.item_id) return { status: "failed", error: "Anexo sem item associado." };
  ...
  try {
    ...
    if (strategy === "plain") {
      ...
      if (hasSufficientTextLayer(layer.text, layer.totalPages)) {
        ...
        text = await ocrPdf(bytes, layer.totalPages, attachment.owner_id, attachment.item_id);
        method = "ai_ocr";
      }
    } else {
