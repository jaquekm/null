...
- **0.4 (tentativa)**: este ambiente remoto **tem Docker** (`docker` e `dockerd` instalados, o daemon sobe normalmente com `dockerd &`), mas não tem o **Supabase CLI**, e o `docker pull` de qualquer imagem falha com `429 Too Many Requests` do Docker Hub (limite de pulls anônimos — não é bloqueio do proxy da organização, o `docker pull hello-world` chega até o registry e leva o 429 de lá mesmo). Ou seja, mesmo instalando o Supabase CLI, `supabase start` provavelmente não conseguiria baixar as ~10 imagens do stack local (Postgres, GoTrue, PostgREST, Realtime, Storage, Kong, Studio...). Não insisti nisso — parei o `dockerd` que tinha subido para teste e seguido para a 0.7, que não depende disso. Continua valendo o plano original: Supabase local roda na sua máquina (`supabase start` via 0.4), não neste ambiente.
